...
<!-- Isi content -->
  ...
  <tbody>   
			<?php
				if(!empty($customers)){ // Jika data pada database tidak sama dengan empty (alias ada datanya)
					$i = 1;
					foreach($customers as $row){ // Lakukan looping pada variabel siswa dari controller
			?>
				<tr>
				<th scope="row"><?= $i; ?></th>
				...
				<td>
					<?= anchor('customer/rekamMedis/'.$row['id_customer'], '<div class="badge badge-primary">rekam</div>') ?>
				</td>
				<td>
					<?= anchor('customer/editCustomer/'.$row['id_customer'], '<div class="badge badge-warning">lihat</div>') ?>
				</td>
				<td onclick="javascript: return confirm('Are you serious for delete this Menu?')">
					<?= anchor('customer/deleteCustomer/'.$row['id_customer'], '<div class="badge badge-danger">delete</div>') ?>
				</td>
			</tr>
			<?php $i++; ?>
			<?php
